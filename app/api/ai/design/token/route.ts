...
import { DESIGN_AGENT_TASK_ID } from "@/lib/trigger/design-agent";
...
export const POST = async (request: Request) => {
  ...
  try {
    ...
    if (taskRun.task_type !== TASK_TYPE_DESIGN_GENERATION) {
    ...
    console.error("Failed to create design agent public token.", error);

    return jsonError(
      500,
      "DESIGN_TOKEN_FAILED",
      "Failed to create design run token.",
    );
  }
};
